...
async function fetchWindow(
  page: number,
  limit: number
): Promise<MALSearchAnimeResponse> {
...
async function fetchAll() {
  for (let i = 81; i <= numAnimeTotal / animePerCall; i++) {
    ...
    response.data.forEach((anime) => {
      ;[anime.title, anime.title_english, ...anime.title_synonyms].forEach(
        (title) => {
          run(
            "INSERT OR IGNORE INTO anime_autocomplete (aka, title, mal_id) VALUES (?, ?, ?)",
            title,
            anime.title,
            anime.mal_id
          )
