<template>
  <Panel
    ...
    @submit="$emit('submit')"
  >
    <template #title> Dot Quota </template>
    <template #description>
      The dot quota for {{ state.count }} {{ state.color }} dice. Only count the
      colored dots.
    </template>
    <template #content>
      <div class="pt-4 h-full">
        <div class="flex flex-wrap text-4xl justify-center px-6 gap-1">
          <div v-for="i in 36" :key="i" class="-mx-1 -my-1">
            <i
              class="bi-dot"
              :style="[
                getAsBoolean(i - 1, state.dots) ? `color: ${state.color};` : '',
              ]"
            ></i>
          </div>
        </div>
  ...
  <Panel
    ...
    @submit="sendSolution"
  >
    <template #title> {{ state.color }} Dice </template>
    <template #description>
    ...
    <template #content>
      ...
      >
        <div v-for="(value, i) in state.dice" :key="i" class="px-6">
          <i
            :class="[`bi-dice-${value} cursor-pointer`]"
            @pointerdown.stop="roll(i)"
          ></i>
        </div>
      </div>
    </template>
