<template>
  <div
    class="bg-gray-900 mx-2 pt-2 relative"
    :class="flip ? '-scale-x-100' : ''"
  >
    <div class="absolute w-6 bg-gray-500 z-20 -top-1 -bottom-1"></div>
    <div class="h-full w-full bg-gray-800 relative">
      <template v-for="(wire, i) in wires" :key="i">
        <div
          class="absolute z-20 bg-gray-900 w-4 h-4 flex items-center rounded-sm cursor-pointer"
          :style="[`top: ${i * 28 - 2}px;`]"
          @click.stop="cutWire(i)"
        >
          <div
            class="h-1 w-4"
            :style="[`background: ${getBackground(wire)}`]"
          ></div>
          <div
            v-if="wire.cut"
            class="absolute left-px right-px h-4 bg-gray-900"
          ></div>
        </div>
        <div
    ...
    <div class="absolute w-2 bg-gray-500 z-20 -top-1 -bottom-1 -right-1"></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    getBackground(wire) {
      return wire.stripe
        ? `repeating-linear-gradient(45deg, ${wire.stripe}, ${wire.stripe} 8px, ${wire.color} 8px, ${wire.color} 16px)`
        : wire.color
    },
    getWireStyle(wire, i) {
      const styles = {}
