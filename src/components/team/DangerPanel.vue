<template>
  <Panel
    ...
    @submit="$emit('submit')"
  >
    <template #title> {{ state.name }} Storage </template>
    <template #description>
      Storage shelf with missing items in Zone {{ state.name }}. The numbers
      indicate how many items should be in each row or column.
    </template>
    <template #content>
      <div class="mt-4">
        <div class="flex text-xl justify-center">
          <div v-for="(num, i) in state.cols" :key="i" class="w-10 text-center">
            {{ num }}
          </div>
        </div>
        <div class="border-4 border-gray-500 mx-10">
          <div v-for="(row, i) in board" :key="i" class="flex justify-center">
            <div class="i text-xl mr-2 absolute left-5">
              {{ state.rows[i] }}
            </div>
            <div
              class="flex justify-center text-center text-amber-600 text-3xl relative"
            >
              <span
                v-for="(item, j) in row"
                :key="j"
  ...
  <Panel
    ...
    @submit="sendSolution"
  >
    <template #title> Storage Zone {{ state.name }} </template>
    <template #description>
      Misplaced packages in Zone {{ state.name }}. Use the arrows to move the
      items back to where they should be.
    </template>
    <template #content>
      <div class="relative top-1/2 -translate-y-2/4">
        ...
        <div class="border-4 border-gray-500 mx-10">
          <div
            ...
            :key="i"
            class="flex justify-center text-center text-amber-500 text-3xl relative"
          >
            <span v-for="(item, j) in row" :key="j" class="cell min-w-10 h-10">
...
export default {
  ...
  methods: {
    tryMove(x, y) {
      const newBoard = Array.from({ length: this.board.length }, () =>
        Array(this.board[0].length).fill(false)
      )
