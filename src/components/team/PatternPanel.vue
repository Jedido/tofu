<template>
  <Panel
    ...
    @submit="$emit('submit')"
  >
    <template #title> Key Pattern </template>
    <template #description>
      The key pattern for the {{ state.color }} {{ state.symbol }} puzzle. Match
      up all colored panels.
    </template>
    <template #content>
      <div
  ...
  <Panel
    ...
    @submit="sendSolution"
  >
    <template #title> {{ state.color }} {{ state.symbol }} Pattern </template>
    <template #description>
      Touch to slide the panels to the empty space. Someone else may have a key.
    </template>
    <template #content>
      <div
        class="relative text-center w-44 mx-auto top-1/2 -translate-y-2/4 text-3xl py-3 px-1 bg-gray-300 rounded"
      >
        <div class="flex flex-col gap-1 px-2">
          <div v-for="(row, i) in board" :key="i" class="flex gap-1">
            <div
              v-for="(_, j) in row"
              :key="j"
              class="h-12 w-12 bg-gray-500 rounded"
            ></div>
          </div>
        </div>
        <template v-for="(row, i) in board" :key="i">
          <template v-for="(lit, j) in row" :key="j">
            <div
              v-if="i !== 2 || j !== 2"
              class="cell h-12 w-12 rounded absolute top-0 left-0 bg-gray-50 cursor-pointer border-2 border-gray-700"
              :style="[
                lit ? `background-color: ${state.color};` : '',
                calculatePosition(j, i),
              ]"
...
export default {
  ...
  data() {
    const boardOffset = Array.from(this.state.board, (row, i) =>
  ...
  methods: {
    ...
    sendSolution() {
      const board = Array.from(this.state.board, (row) =>
