<template>
  <PanelWrapper
    ...
    @submit="$emit('submit')"
  >
    <template #title> Q{{ state.n }} Cheat Sheet </template>
    <template #description>
      Solve for the requested variable. It will be used in a later equation.
    </template>
    <template #content>
      <div
        class="h-full text-4xl flex flex-col justify-center pb-2 gap-1 text-gray-600"
      >
        <div
          v-for="(equation, i) in equations"
          :key="i"
          class="pl-6 border-b-2 border-cyan-300"
        >
  ...
  <PanelWrapper
    ...
    @submit="sendSolution"
  >
    <template #title> Question #{{ state.n }} </template>
    <template #description>
      Solve for {{ state.y }}.
      <span v-if="state.hint"
        >Since this is the first equation, you have additional information.
        Please do not forget your answer.</span
      >
    </template>
    <template #content>
      <div
        class="h-full font-serif text-4xl text-center flex flex-col justify-around"
      >
        <div>
          <div v-if="state.hint" class="mb-3">0. {{ state.hint }}</div>
          {{ state.n }}. {{ state.y }} = ?
        </div>
        <div class="shrink flex justify-center">
          <i
            class="bi-dash-circle text-3xl leading-relaxed text-error cursor-pointer"
            @pointerdown.stop="increment(-1)"
          ></i>
          <span class="text-4xl text-center w-16">{{ number }}</span>
          <i
            class="bi-plus-circle text-3xl leading-relaxed text-emerald-500 cursor-pointer"
            @pointerdown.stop="increment(1)"
          ></i>
        </div>
      </div>
